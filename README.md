...
- **Dashboard rail (persistent)** — the coach docks a gentle-shell-style card rail automatically in fullscreen terminals: it appears whenever the terminal is wide enough per gentle-shell's breakpoint (≥ 140 columns) and live-updates from the log/vocab files after every assistant message. The slim, error-focused content shows the top recurring corrections (up to 4) each with a one-line practice recommendation derived from the error, a compact vocabulary section (`N due now` plus up to 2 due phrases), and a single Recommendations hint line; the weekly trend stays visible in the card subtitle and `/language stats`. The rail collapses/expands via the title control (click `▾ Collapse` / `▸ Expand`, or press `ctrl+shift+l`), showing a compact summary line when collapsed. `/language panel` + **alt+c** remain as the fallback overlay for narrow or non-fullscreen terminals.
